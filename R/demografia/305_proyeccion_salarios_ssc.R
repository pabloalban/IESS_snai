...
load( file = paste0( parametros$demo_rdata_ssc_incom_tran_act_anio, parametros$anio_ini, '.RData' ) )

# Parámetros ---------------------------------------------------------------------------------------
message( '\tPreparando parámetros' )

# Horizonte de proyección
t_max <- parametros$ssc_horizonte

anio_ini <- parametros$anio_ini

sexo_lst <- factor( c( 'H', 'M' ), levels = c( 'H', 'M' ) )
t_lst <- seq( 0, t_max )
x_lst <- seq( 0, parametros$demo_edad_max )
s_lst <- seq( 0, parametros$demo_ts_max )
...
tas_macro_sex_edad[ , u_r := 1 ]  #Porque mi salario es la proyección del SBU
...
sal_proy <- ssc_comp_tran[ x >= 15, list( sexo, imp, x, ER_act, S ) ]
...
sal_proy_x <- ssc_comp_tran[ x >= 15, list( sexo, x, ER_act, S ) ]
...
# # Proyección salarios ------------------------------------------------------------------------------
# message( '\tGenerando proyección de salarios' )
# anio_max <- parametros$anio_ini
# 
# # Preparando tasa para la proyección
# tas_macro_sex_edad <- copy( esc$apo_act[ , list( t, anio, sbu ) ] )
# 
# aux <- data.table( expand_grid(
#   t = 0:parametros$ssc_horizonte,
#   sexo = factor( c( 'H', 'M' ), levels = c( 'H', 'M' ) ),
#   x = 15:105 ) )
# aux[ , anio := t + parametros$anio_ini ]
# 
# sal_proy <- merge.data.table( aux, tas_macro_sex_edad, by=c( 't', 'anio' ), all.x=T )
# sal_proy <- sal_proy[ , list( anio, t = as.integer( anio - min( sal_proy$anio ) ), sexo, x, sal = sbu ) ]
# 
# # Guardando resultados -----------------------------------------------------------------------------
# message( '\tGuardando resultados de proyección e salarios' )
# save( sal_proy, file = esc$rdata_sal_proy )
