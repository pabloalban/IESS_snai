...
X <- as.matrix( onu_ecu_surv[ sexo == 'F' & t > 2000, list( t, x ) ] )
Z_f <- as.matrix( onu_ecu_surv[ sexo == 'F' & t > 2000, list( log_u0 ) ] )
Z_m <- as.matrix( onu_ecu_surv[ sexo == 'M' & t > 2000, list( log_u0 ) ] )
Y <- as.matrix( expand.grid( t = 2000:2100, x = 0:110 ) )
...
NLM_f <- nlfb( start =  c( 2500, 10000 ), resfn = fit_variogram, jacfn = NULL, 
               trace = TRUE, weights = NULL, lower = c( 0, 1 ), upper = c( 5e3, 20e3 ), 
               data = data.frame( d = d_f, V = V_f ), d = d_f, V = V_f, 
               control = list( watch = FALSE, femax = 100, ndstep = 1e-6 ) )
str( NLM_f )

NLM_m <- nlfb( start =  c( 4.674695, 7696 ), resfn = fit_variogram, jacfn = NULL, 
               trace = TRUE, weights = NULL, lower = c( 0, 1 ), upper = c( 5e3, 20e3 ), 
               data = data.frame( d = d_m, V = V_m ), d = d_m, V = V_m, 
               control = list( watch = FALSE, femax = 50, ndstep = 1e-6 ) )
str( NLM_m )

Kern_f <- function( x, y ) gaussian_kernel( dist( x, y ), NLM_f$coefficients[1], NLM_f$coefficients[2] )
Kern_m <- function( x, y ) gaussian_kernel( dist( x, y ), NLM_m$coefficients[1], NLM_m$coefficients[2] )
...
onu_ecu_mort_din[ , log_ux := log( ux ) ]
...
save( d_f, V_f, KRIG_f, NLM_f,
      ...
      file = paste0( parametros$RData, 'ONU_kriging_life_table_survivors_2019_v2.RData' ) )
